...
template <typename Iter, typename Iter_end>
void conv(Iter signal1start, Iter_end signal1end, Iter signal2start, Iter_end signal2end, 
            Iter outstart, Iter_end outend){
...
template <typename Iter, typename Iter_end>
void conv_fft(Iter signal1start, Iter_end signal1end, Iter signal2start, Iter_end signal2end,
                Iter outstart, Iter_end outend){
...
int main(){
    ...
    conv(signal1.begin(), signal1.end(), signal2.begin(), signal2.end(), out1.begin(), out1.end());
    conv_fft(signal3.begin(), signal3.end(), signal4.begin(), signal4.end(), out2.begin(), out2.end());
    ...
    for (int i = 0; i < signal1.size(); ++i) {
        std::cout
        << std::setw(16) << i
        << std::setw(16) << (std::abs(out1[i]) - std::abs(out2[i]))
        << '\n';
    }
