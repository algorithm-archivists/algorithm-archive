...
In this chapter, we will show you how to make some of the most famous fractals via Iterated Function Systems (IFSs): the Sierpinski triangle.
...
Though this is an incredibly interesting concept, the discussion of this chapter will focus primarily on methods to generate fractal patterns and will refrain from delving into this discussion for now and instead focus on the method by which fractals can be generated through IFSs.
...
For example, let's use all three functions on all three existing points, which will create 9 additional points, many of which overlap.
...
If we then create 4 initial points located on the edges of the square, halfway between the vertices and allow these points to spawn 4 children each, something peculiar happens:

ADD ANIMATION

It would seem that the final result is just a random distribution of points:

ADD IMAGE

What happened to the self-similar structure we were getting before?
Why isn't this more interesting?
These are very good questions that I don't know the full answer to myself.
If you have a satisfying answer, please reach out and let me know, so I can update the text here!
...
Every iteration, we generate 3 or 4 new children, and they generate the same number of children, and so on.
...
Here, instead of tracking children of children, we track a single individual that chooses randomly between the Hutchinson functions.
For the Sierpinski triangle above, the code would look like this:

ADD CODE

After a few thousand iterations, it becomes possible to see the fractal again:

ADD ANIMATION (OR SET OF IMAGES)
...
If we start the point at the center of the triangle, it will eventually find it,s way onto the triangle, and then all subsequent iterations will be on the fractal.
Here, I have plotted the steps where the wanderer is still wandering around, looking for the correct shape:
...
So let's go back to the square.
Before, we saw a random distribution of points.
...
Discussing chaos games in more detail is a chapter in its own right, so I will forego the discussion here.
