...
#val(x,y) = (abs(f(z(x,y)))-floor(abs(f(z(x,y)))))
color(x,y) = hsv2rgb(angle(rf(z(x,y)), if(z(x,y))), val(x,y), shade(x,y))
#color(x,y) = hsv2rgb(angle(rf(z(x,y)), if(z(x,y))), val(x,y), 1)
...
#shade(x,y) = abs(sin(if(z(x,y))*pi)**thresh)
save_encoding = "utf8"
