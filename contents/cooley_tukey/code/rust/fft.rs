...
fn dft(x: &Vec<Complex<f64>>) -> Vec<Complex<f64>> {
    let n = x.len();
    // TODO: replace this with something that uses an iterable
    let mut temp = vec![Complex::new(0.0_f64, 0.0_f64); n];
    for i in 0..n {
        for k in 0..n {
            temp[i] += x[k]
                * (Complex::new(0.0_f64, -2.0_f64) * PI * (i as f64) * (k as f64) / (n as f64))
                    .exp();
        }
    }
    temp
...
fn cooley_tukey(x: &Vec<Complex<f64>>) -> Vec<Complex<f64>> {
    let n = x.len();
    if n <= 1 {
        return x.clone();
    }
...
fn bit_reverse(x: &Vec<Complex<f64>>) -> Vec<Complex<f64>> {
    let n = x.len();
    let mut temp = vec![Complex::new(0.0_f64, 0.0_f64); n];
    for k in 0..n {
...
fn iterative_cooley_tukey(x: &Vec<Complex<f64>>) -> Vec<Complex<f64>> {
    let n = x.len();
    ...
    for i in 1..=((n as f64).log2() as usize) {
        ...
        for j in (0..n).into_iter().step_by(stride as usize) {
            ...
            for k in 0..((stride / 2) as usize) {
                ...
                new_x[k + j] -=
                    new_x[k + j + ((stride / 2) as usize)].clone() - new_x[k + j].clone();
                v *= w;
            }
        }
    }
